  ...
  return (
    <section>
      <div className='w-[100vw] h-[100vh] flex justify-center items-center absolute bottom-[13vh]'>
        <img className='opacity-[70%] absolute z-[0] m-auto' src={krishna} alt='krishna' />
      </div>
      <div className='absolute pl-[85vh] pt-[27rem]'>
        <img className='backdrop-opacity-5 absolute w-[25rem] z-[0] top-[30rem]' src={logounder} alt="logo"></img>
        <p className='text-[5rem] font-[Alegreya]'>VedicVani</p>
      </div>
      <div className='flex justify-center items-center w-[40%] m-auto pt-[37rem]'>
        <form className='w-[100%] flex justify-evenly items-evenly pr-[3rem]'>
          <select
          ...
          <Link to={`/api/chapter/${selectedchap}/shlok/${selectedcvers}`}>
            <button className='bg-[#002147] text-[#F2F3F4] font-[Alegreya] text-[2rem] rounded-[1rem] absolute w-[100px] duration-[0.3s] left-[80rem] hover:bg-[#F2F3F4] hover-text-[#002147]'>Go</button>
          </Link>
        </form>
      </div>
      <div className='flex justify-center items-center w-[100vw]'>
        <Link to={`/api/chapter/1/shlok/1`}>
          <button className='bg-[#002147] text-[#F2F3F4] font-[Alegreya] text-[2rem] rounded-[1rem] px-[2rem] py-[1rem] absolute top-[50rem] w-[400px] duration-[0.3s] left-[30rem] hover-bg-[#F2F3F4] hover-text-[#002147]'>Read Shlok's</button>
        </Link>
        <Link to={`/api/chapter/1`}>
          <button className='bg-[#002147] text-[#F2F3F4] font-[Alegreya] text-[2rem] rounded-[1rem] px-[2rem] py-[1rem] absolute top-[50rem] w-[400px] duration-[0.3s] right-[30rem] hover-bg-[#F2F3F4] hover-text-[#002147]'>chapters's summary</button>
        </Link>
      </div>
    </section>
  );
